...
template <typename floatTypeT, typename intTypeT>
FldHeader<floatTypeT, intTypeT>::FldHeader(const std::string& filename) : Filename(filename)
{
  ...
  if (pressMeshKey == "t" || pressMeshKey == "T") {
    throw std::runtime_error("Error: .fld file " + Filename
      + "specifies if_press_mesh=F, but PnPn-2 is not supported.");
  }
